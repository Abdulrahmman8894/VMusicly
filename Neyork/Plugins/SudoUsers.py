...
from Neyork import BOT_ID, MUSIC_BOT_NAME, OWNER_ID, SUDOERS, app
from Neyork.Database import (add_gban_user, add_off, add_on, add_sudo,
...
/delmsudo [Username or Reply to a user]
...
async def useradd(_, message: Message):
    if not message.reply_to_message:
        ...
        if added:
            ...
            os.system(f"kill -9 {os.getpid()} && python3 -m Neyork")
        else:
...
@app.on_message(filters.command("delmsudo") & filters.user(OWNER_ID))
async def userdel(_, message: Message):
    if not message.reply_to_message:
        ...
        if removed:
            ...
            return os.system(f"kill -9 {os.getpid()} && python3 -m Neyork")
