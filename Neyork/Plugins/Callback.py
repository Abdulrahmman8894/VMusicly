...
from Neyork import BOT_USERNAME, MUSIC_BOT_NAME, app, db_mem
from Neyork.Core.PyTgCalls import Queues, Neyork
from Neyork.Core.PyTgCalls.Converter import convert
from Neyork.Core.PyTgCalls.Downloader import download
from Neyork.Database import (_get_playlists, delete_playlist, get_playlist,
                            get_playlist_names, is_active_chat, save_playlist)
from Neyork.Database.queue import (add_active_chat, is_active_chat,
                                  is_music_playing, music_off, music_on,
                                  remove_active_chat)
from Neyork.Decorators.admins import AdminRightsCheckCB
from Neyork.Decorators.checker import checkerCB
from Neyork.Inline import (audio_markup, audio_markup2, download_markup,
                          fetch_playlist, paste_queue_markup, primary_markup)
from Neyork.Utilities.changers import time_to_seconds
from Neyork.Utilities.chat import specialfont_to_normal
from Neyork.Utilities.paste import isPreviewUp, paste_queue
from Neyork.Utilities.theme import check_theme
from Neyork.Utilities.thumbnails import gen_thumb
from Neyork.Utilities.timer import start_timer
from Neyork.Utilities.youtube import get_yt_info_id
...
async def admin_risghts(_, CallbackQuery):
    ...
    if command == "pausecb":
        ...
        await Neyork.pytgcalls.pause_stream(chat_id)
    ...
    if command == "resumecb":
        ...
        await Neyork.pytgcalls.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
    ...
    if command == "stopcb":
        ...
        await Neyork.pytgcalls.leave_group_call(chat_id)
        await CallbackQuery.message.reply_text(
    ...
    if command == "skipcb":
        ...
        if Queues.is_empty(chat_id):
            ...
            if str(finxx) != "raw":
                ...
                await Neyork.pytgcalls.change_stream(
                    chat_id,
                    InputStream(
                        InputAudioStream(
                            raw_path,
    ...
    if not _playlist:
        ...
        for shikhar in _playlist:
            ...
            if await is_active_chat(chat_id):
                ...
                raw_path = await convert(downloaded_file)
                try:
                    await Neyork.pytgcalls.join_group_call(
                        chat_id,
                        InputStream(
                            InputAudioStream(
                                raw_path,
